...
3. **Environment variables** (Settings → Environment Variables):
   - `NEXT_PUBLIC_SITE_URL` = `https://<your-domain>` (no trailing slash). Drives canonical URLs, hreflang, sitemap and Open Graph. Until a custom domain exists the code falls back to Vercel's production URL (`VERCEL_PROJECT_PRODUCTION_URL`), so the `.vercel.app` deployment is already consistent; set the variable the day the domain goes live and redeploy.
